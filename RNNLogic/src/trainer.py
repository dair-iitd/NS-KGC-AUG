...
class TrainerPredictor(object):
    ...
    def __init__(self, model, train_set, valid_set, test_set, optimizer=None, scheduler=None, gpus=None, num_worker=0):
    ...
    @torch.no_grad()
    def evaluate(self, split, filter=None, expectation=True):
        if comm.get_rank() == 0:
