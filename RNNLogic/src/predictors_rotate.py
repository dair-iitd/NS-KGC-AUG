...
class PredictorPlus(torch.nn.Module):
    def __init__(self, graph, type='emb', num_layers=3, hidden_dim=16, entity_feature='bias', aggregator='sum', embedding_path=None, rel_weight=0.001):
        ...
        if self.type == 'lstm':
            ...
            self.rnn = MLP(self.hidden_dim, [self.hidden_dim, self.hidden_dim])
        elif self.type == 'emb':
            self.rule_emb = None
        else:
    ...
    def get_rotate_scores(self, all_h, r_head):
        h_emb = self.rotate.eemb.index_select(0, all_h).squeeze()
        rule_emb = torch.zeros(1, self.rotate.emb_dim, device=all_h.device)
        r_emb = self.rotate.remb[r_head]
        rule_emb = rule_emb + r_emb
        rule_emb = self.rotate.project(rule_emb)
        h_emb = self.rotate.product(h_emb, rule_emb)
        t_emb = self.rotate.eemb.index_select(0, torch.arange(self.num_entities, device=all_h.device)).squeeze()
    ...
    def forward(self, all_h, all_r, edges_to_remove):
        ...
        if self.entity_feature == 'bias':
            ...
            rotate_bias = self.get_rotate_scores(all_h, query_r)
            score = score + (self.rel_weight * rotate_bias)
            mask = torch.ones_like(mask).bool()
        else:
            mask = (mask != 0)
            score = score.masked_fill(~mask, float('-inf'))
